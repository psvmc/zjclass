$(function () {
    ...
    $(".comment_appraise_body").height($("#content").height() - 6.2 * j_size - $(".homework_assigned_list").height());
    ...
    $(".job_information_main_operation .full").click(function () {
        if ($(this).text() == "全屏") {
            ...
            $(".lecture_main").height($("body").height() - 12);
            $(".homework_assigned_list").css({
                "display": "none",
                "height": 0
            });
            $(".prepare_head").css("display", "none");
            $(".comment_appraise_main").height($("#content").height() - 12);
            $(".comment_appraise_body").height($("#content").height() - $(".homework_assigned_list").outerHeight() - parseInt($(".homework_assigned").css("marginTop")) - parseInt($(".comment_appraise_body").css("marginTop")) - 14);
        } else {
            $(".main").removeClass("all_screen");
            $(this).html("<img src=\"img/comment/full.png\"/>全屏");
            $(".prepare_head").css("display", "block");
            $(".lecture_main").height($("#content").height() - 12);
            $(".homework_assigned_list").css({
                "display": "block",
                "height": "auto"
            });
            $(".comment_appraise_body").height($("#content").height() - $(".prepare_head").outerHeight() - $(".homework_assigned_list").outerHeight() - parseInt($(".homework_assigned").css("marginTop")) - parseInt($(".comment_appraise_body").css("marginTop")) - 14);
        }
...
new Vue({
    ...
    watch: {
        selectindex: function () {
            if (this.isshow_arr.length > this.selectindex) {
                this.isshow = this.isshow_arr[this.selectindex];
            } else {
                this.isshow = false;
            }
        }
    ...
    filters: {
        ...
        rightratefilter: function (ques) {
            var scount = ques.scount;
            var ccount = ques.ccount;
            if (scount == 0) {
                return 0;
            } else {
                return (parseFloat(ccount) * 100 / scount).toFixed(1);
            }
        },
    ...
    mounted: function () {
        this.loadData();
        window.analysistiming = setInterval(this.loadData, 5000);
    },
    methods: {
        show_or_hide_click: function () {
            if (this.isshow_arr.length > this.selectindex) {
                var show_hide = this.isshow_arr[this.selectindex];
                this.isshow_arr[this.selectindex] = !show_hide;
                this.isshow = !show_hide;
            }
        },
        rightrate_error_show: function (ques) {
            var scount = ques.scount;
            var ccount = ques.ccount;
            if (scount == 0) {
                return true;
        ...
        loadData: function () {
            var _this = this;
            classapi.post.teacherclass_paper_question_studentaccuary({
                ...
                if (classapi.validata(result)) {
                    ...
                    if (_this.isshow_arr.length == 0) {
                        for (var i = 0; i < obj.length; i++) {
                            _this.isshow_arr.push(false);
                        }
                    }
